...
3. **`BedServer.java`의 통신 규칙을 바꾸지 않는다.** 침대 펌웨어는 고칠 수 없다. 바이트 하나 틀리면 침대가 붙지 않는다. (기록 문구를 늘리는 것은 괜찮다 — 5.9.3 의 동기화 요청 기록처럼. 바꾼 뒤 `test/pairing` 을 돌려 가짜 침대 로그인·명령이 그대로인지 확인)
